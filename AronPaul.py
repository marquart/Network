...
def standardMenu(Parent):
	...
	newMenu = Menu(Parent)
	myMenu.add_cascade(label="New..", menu=newMenu)
	newMenu.add_command(label="Conections", command=guiCon)
	newMenu.add_separator()
	newMenu.add_command(label="Exit")
	
	showMenu = Menu(Parent)
	myMenu.add_cascade(label="Show..", menu=showMenu)
	showMenu.add_command(label="Conections", command=createNetwork)
	showMenu.add_separator()
	showMenu.add_command(label="Exit")
...
def createNetwork():
	g = nx.read_edgelist('EdgeList.csv',delimiter=',', create_using=nx.Graph(), nodetype=str)
	nx.draw(g)
	plt.show(g)
